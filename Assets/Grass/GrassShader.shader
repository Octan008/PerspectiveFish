Shader "Custom/SimpleGeometryShaderAndGrassShader/Grass" {
	...
	SubShader
	{
		...
		Pass
		{
			...
			{
				float offset = 1.0;
				float4 p0 = i[0].pos + offset * i[0].rot;
				float4 p1 = i[1].pos+ offset * i[0].rot;
				float4 p2 = i[2].pos+ offset * i[0].rot;

				float3 n0 = i[0].nor;
				float3 n1 = i[1].nor;
				float3 n2 = i[2].nor;

				float height =  sigmoid((i[0].hei.r + i[1].hei.r + i[2].hei.r) / 3.0f);
				...
				float4 dir2 = float4(normalize((p1 - center) * rot).xyz, 1.0f);
				...
				o[2].pos += dir * (_WindPower * wind * _BottomBend) * sin(10*_Time);
				o[3].pos += dir * (_WindPower * wind * _BottomBend) * sin(10*_Time);
				o[4].pos += dir * (_WindPower * wind * _MiddleBend) * sin(10*_Time);
				o[5].pos += dir * (_WindPower * wind * _MiddleBend) * sin(10*_Time);
				o[6].pos += dir * (_WindPower * wind * _TopBend) * sin(10*_Time);
				...
				stream.RestartStrip();
				dir = float4(normalize((p2 - p0) * rot).xyz, 1.0f);
				dir2 = float4(normalize((p1 - center) * rot).xyz, 1.0f);
				...
				o[1].pos = center + dir2 * bottomWidth;
				o[1].color = _BottomColor;
				...
				o[3].pos = center + dir2 * middleWidth + normal * bottomHeight;
				o[3].color = lerp(_BottomColor, _TopColor, 0.33333f);

				// Middle to Top.
				o[4].pos = tmp3 - dir * topWidth + normal * middleHeight;
				o[4].color = lerp(_BottomColor, _TopColor, 0.66666f);

				o[5].pos = tmp3 + dir2 * topWidth + normal * middleHeight;
				o[5].color = lerp(_BottomColor, _TopColor, 0.66666f);

				// Top.
				o[6].pos = tmp + dir * topWidth + normal * topHeight;
				o[6].color = _TopColor;
				...
				// Bottom.
				o[0].pos = center + dir2 * bottomWidth;
				o[0].color = _BottomColor;
				...
				// Bottom to Middle.
				o[2].pos = center + dir2 * middleWidth + normal * bottomHeight;
				o[2].color = lerp(_BottomColor, _TopColor, 0.33333f);
				...
				// Middle to Top.
				o[4].pos = o[3].pos + dir2 * topWidth + normal * middleHeight;
				o[4].color = lerp(_BottomColor, _TopColor, 0.66666f);
			...
			fixed4 frag(g2f i) : COLOR
			{
				...
				i.color = fixed4(i.color.xyz*distance, 1);
